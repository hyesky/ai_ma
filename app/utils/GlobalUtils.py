...
__log_name = "%s%s.log" % ("ai_ma", datetime.now().strftime("%Y%m%d-%H%M%S"))
...
class CheckServerUtils():
    ...
    def checkVersion(request_ip, peer_ip, peer_port, lang=None):
        # [ai_ma] 已禁用回连：不再向原作者服务器上报机器信息/检查版本
        __online = False
        __state = False
        __msg = LANG_VIEWS_USE_LANG_T(lang, "msg_unknown_error")
        __info = {}
        ...
        try:
            servicesHost = ""  # [ai_ma] 回连已禁用，原作者服务地址已清除

            url = "%s/api/ai_ma/checkVersion" % servicesHost
            osSystem = OSSystem()
    ...
    def reportHeart(report_count, lang=None):
        ...
        try:
            servicesHost = ""  # [ai_ma] 回连已禁用，原作者服务地址已清除
            url = "%s/api/ai_ma/reportHeart" % servicesHost
